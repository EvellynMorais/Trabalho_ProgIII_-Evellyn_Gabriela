...
app.get('/api/musica', (req, res) => {
    res.json({
        nome: beatriz.nome,
        artista: beatriz.artista,
        letraInteira: beatriz.getLetraInteira(),
        partes: beatriz.partes
    });
});
...
app.get('/api/partes', (req, res) => {
    ...
    if (tag) {
        const filtradas = beatriz.partes.filter(p => p.tag === tag);
        return res.json(filtradas);
    }

    // sem filtro → retorna todas
    res.json(beatriz.partes);
});
...
app.post('/api/partes', (req, res) => {
    ...
    beatriz.addParte(novaParte);

    res.status(201).json({
        mensagem: 'Parte adicionada',
        parte: novaParte,
        totalPartes: beatriz.partes.length
    });
});
...
<head>
    ...
    <title>${beatriz.nome} — ${beatriz.artista}</title>
    <style>
